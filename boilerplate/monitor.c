...
struct monitored_entry {
    ...
    int soft_warned;          /* 1 after we have emitted the soft warning */
 ...
 *
 * We use a mutex (not a spinlock) because both the timer callback
 * (softirq / workqueue context on modern kernels — can sleep) and
 * the ioctl handler (process context — can sleep, and calls
 * kmalloc(GFP_KERNEL)) need to be able to sleep while holding the
 * lock.  A spinlock holder must never sleep, so a spinlock would
 * be incorrect here.
 * ============================================================== */
...
static void timer_callback(struct timer_list *t)
{
     ...
     *
     * We use list_for_each_entry_safe so we can safely delete the
     * current node during iteration without corrupting the list walk.
     * ============================================================== */
